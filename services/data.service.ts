...
export abstract class DataCrudService<T extends ApiResource> {
  ...
  createItem(item: T): Observable<T> {
    return this.http.post<T>(this.getCommonUrl(), item, { headers: this.httpHeaders });
  }

  updateItem(id: any, item: T): Observable<T> {
    const url = `${this.getCommonUrl()}/${id}`;
